...
| **agentic** (multi-turn tool loop) | **~12%** (run-variable 7–22% over 7 runs) | `--workload agentic` |
...
`--workload full` also prints a **disclosed illustrative-mix blend** (~32% at the default balanced weights, tracking the agentic lever's spread; `--weights` tunable) — a weighted average of the reproducible parts, echoed with its weight vector into `ab_results.json`. It is **not** a second headline, and it lands **below 54.1% by construction — which is the honest point**: (a) the largest agentic lever (G14/G15 tool-output projection) **structurally cannot fire in a live single-loop A/B** — only tool-catalogue pruning (G08/G16) is live-reproducible here, so agentic reads ~12% vs the internal 46%; **this figure was corrected downward on 2026-09-06** — the harness had been pinning G16's system-prompt cap to 800 tokens against a shipped default of 4096, and every BFCL episode carries a ~1,046-token system prompt, so a lever no default install applies was worth roughly half the number and also damped its variance; it now runs the shipped config; and (b) the recognized public prose items are **small and stateless** (~2–4%), where the internal 38% prose figure comes from production-scale documents — so a disclosed **production-shaped `ops` profile** (verbose DevOps payloads, flagged in [`DATA_LICENSES.md`](examples/benchmark/DATA_LICENSES.md) as *not* a recognized benchmark) is included to exercise the structured-pruning levers (G19/G22) that only fire on bulky first-asks, lifting the prose lever to ~8%. A skeptic reproduces each *part*, sees exactly why the blend differs from the headline, and trusts both numbers more — nothing is tuned to land on a target. Already on a managed proxy? Preview savings against your **live** deployment — see [`docs/client-onboarding.md`](docs/client-onboarding.md).
...
| **G2** | Template Registry | none (registry + budget reporting) | Versioned templates with PR-diff token checks, deprecation metadata and a budget WARNING when a request exceeds a template's `total_input_max`. Read-only: it never edits the request (its opt-in system-prompt truncation was removed 2026-09-08 because it changed answers; runtime budget compaction is G26) |
...
| **G2** Template Registry | `budgets.<id>.{system_prompt_max, total_input_max}` (`system_prompt_max` is a build-time CI check) | *(no token effect - G2 only reports an over-budget request)* | n/a |
...
| **G11** Output Format | `max_tokens_auto_tighten` false; `tighten_quantile` 0.95; `tighten_multiplier` 2.0; `fallback_max_tokens` null | lower quantile/multiplier → tighter caps; set `fallback_max_tokens` to cap cold starts | caps are derived only from past answers to the SAME workflow or template, never from a tenant's other traffic; a request that identifies neither is never capped, and the loop ships off |
| **G12** Reasoning Budget | `default_effort` medium; `effort_map.*` per-provider thinking budgets | `default_effort: off` — no reasoning parameter at all | raise effort for hard reasoning tasks |
...
| **G28** Context Reuse (CCR) | `enabled` false; `min_tokens` 300; `require_proven_resolver` true; `compress_system_prompt` false | enable for agent clients; lower min | keep `require_proven_resolver` **on** — it is what stops a reference reaching a client that cannot resolve it |
...
| Managed self-service console + billing dashboards + request-count invoicing | — | ✅ tabbed **Operator Console** (Tenants · Observability · Billing) with a per-tenant management drawer, all-tenant invoice run, cross-tenant trust-&-safety report, and one-click BYOK key re-encryption |
