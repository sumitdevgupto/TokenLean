...
3. **summarize** — ⚠ **lossy** — one cheap-model (`summary_model`) summary replaces the whole old span as a single `system` message. Summaries are cached per conversation prefix for `summary_ttl_seconds`, so later turns of the same conversation reuse them instead of paying for a second summarisation. **A summary keeps the gist, not every detail:** a specific value buried in the compacted span (an id, a code, a time window) can be summarised away, and the model will then answer *without* it — confidently, on a normal HTTP 200. Measured 2026-09-03: on a compacted ~12k-char policy document, rungs `compress+summarize` alone (with `rungs.drop` **off**, the shipped default) cut 2,588 → 259 tokens and the model could no longer answer a question about a detail that existed only in that span. Rung 4 is not the only lossy rung — it is only the *most* lossy. If a conversation's older turns contain details that must survive, raise `keep_recent_turns` or `target_pct`, or park the content with **G28** instead, whose reference is fetched back verbatim.
...
| `rungs.summarize` | `true` | ⚠ Rung 3 — **lossy** — cheap-model summary of the old span (cached). Keeps the gist; a specific value inside the compacted span can be summarised away |
